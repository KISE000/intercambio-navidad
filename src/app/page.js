...
export default function Home() {
  ...
  const getUserAvatarStyle = () =>
    session?.user?.user_metadata?.avatar_style || "robot";
  const getUserAvatarSeed = () =>
    session?.user?.user_metadata?.avatar_seed || session?.user?.email;
  ...
  const daysLeft = selectedGroup?.event_date
    ? getDaysLeft(selectedGroup.event_date)
    : null;
  ...
  const handleBugReport = () => {
    window.location.href =
      "mailto:soporte@ishop.com?subject=Reporte Bug iShop Navidad&body=Hola, encontré el siguiente error:";
    setIsMenuOpen(false);
    setIsMobileMenuOpen(false);
  };
  ...
  const myWishesCount = wishes.filter(
    (w) => w.user_id === session?.user?.id
  ).length;
  const othersWishesCount = wishes.filter(
    (w) => w.user_id !== session?.user?.id
  ).length;
  ...
  if (loading)
    return (
      <div className="min-h-screen bg-background flex items-center justify-center text-purple-500 relative overflow-hidden">
        <span className="animate-pulse z-10 font-mono">
          Iniciando sistemas...
        </span>
      </div>
    );
    ...
    <div className="min-h-screen bg-background text-text-main font-sans pb-20 relative selection:bg-purple-500/30 transition-colors duration-300">
      ...
      <header className="sticky top-0 z-50 bg-surface/70 backdrop-blur-xl border-b border-white/10 shadow-lg shadow-purple-500/5 px-4 md:px-6 py-4 flex justify-between items-center transition-all duration-300">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-purple-600 to-indigo-600 flex items-center justify-center text-xl shadow-lg shadow-purple-500/20">
            🎄
          </div>
          <div>
            <h1 className="text-lg font-bold text-text-main tracking-tight leading-none">
              {selectedGroup.name}
            </h1>
            <div
              className="flex items-center gap-2 text-xs text-text-muted mt-1 cursor-pointer group/code"
              onClick={handleInvite}
              title="Click para copiar invitación"
            >
        ...
        <div className="hidden md:block relative" ref={menuRef}>
          <button
            ...
            className={`flex items-center gap-3 bg-surface hover:bg-surface-highlight border border-border rounded-full pl-1.5 pr-5 py-1.5 transition-all duration-300 group ${
              isMenuOpen ? "ring-2 ring-purple-500/20 border-purple-500/40" : ""
            }`}
          >
            <Avatar
              seed={getUserAvatarSeed()}
            ...
            <div className="flex flex-col items-start leading-none">
              <span className="text-xs font-bold text-text-main group-hover:text-purple-400 transition-colors">
                Mi Cuenta
              </span>
            ...
            <span
              className={`text-[10px] text-text-muted ml-2 transition-transform duration-300 ${
                isMenuOpen ? "rotate-180 text-purple-400" : ""
              }`}
            >
              ▼
            </span>
          ...
          {isMenuOpen && (
            <div className="absolute right-0 top-full mt-3 w-72 glass-menu rounded-2xl overflow-hidden animate-menu-in z-50 origin-top-right">
              <div className="p-5 border-b border-border bg-surface-highlight/30">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-purple-400 font-bold uppercase tracking-wider mb-0.5">
                      Conectado como
                    </p>
                    <p
                      className="text-sm text-text-main font-medium truncate"
              ...
              <div className="p-2 space-y-1">
                {isAdmin && (
                  <>
                    <h4 className="text-[9px] text-yellow-500/70 font-mono uppercase tracking-widest px-2 pt-1 pb-1">
                      ADMIN TOOLS
                    </h4>
                    <button
                      onClick={() => {
                    ...
                    >
                      <span className="menu-icon-box text-cyan-400 group-hover:bg-cyan-500/20 group-hover:text-cyan-300">
                        ⚙️
                      </span>
                ...
                <h4 className="text-[9px] text-text-muted font-mono uppercase tracking-widest px-2 pt-1 pb-1">
                  MI CUENTA
                </h4>
                <button
                  onClick={() => {
                  ...
                  className="menu-item group"
                >
                  <span className="menu-icon-box text-emerald-400 group-hover:bg-emerald-500/20 group-hover:text-emerald-300">
                    🎫
                  </span>
                ...
                <button
                  ...
                  className="menu-item group"
                >
                  <span className="menu-icon-box text-purple-400 group-hover:bg-purple-500/20 group-hover:text-purple-300">
                    🎨
                  </span>
                ...
                <button onClick={handleInvite} className="menu-item group">
                  <span className="menu-icon-box text-blue-400 group-hover:bg-blue-500/20 group-hover:text-blue-300">
                    🔗
                  </span>
                ...
                <button
                  ...
                  className="menu-item group"
                >
                  <span className="menu-icon-box text-indigo-400 group-hover:bg-indigo-500/20 group-hover:text-indigo-300">
                    ⚖️
                  </span>
                ...
                <button
                  ...
                  className="menu-item group"
                >
                  <span className="menu-icon-box text-slate-400 group-hover:bg-slate-500/20 group-hover:text-slate-200">
                    🔄
                  </span>
                ...
                <button onClick={handleBugReport} className="menu-item group">
                  <span className="menu-icon-box text-orange-400 group-hover:bg-orange-500/20 group-hover:text-orange-300">
                    🐛
                  </span>
                ...
                <button
                  ...
                  className="menu-item group mt-2 border border-red-500/20 bg-red-500/5 text-red-400 hover:bg-red-500/10"
                >
                  <span className="menu-icon-box bg-transparent border-none text-red-400 shadow-none">
                    🚪
                  </span>
                  ...
                  <div className="w-full p-6 flex flex-col md:flex-row items-center justify-between gap-6 bg-gradient-to-r from-yellow-900/10 to-transparent animate-in zoom-in duration-300">
                    <div className="flex items-center gap-4">
                      <div className="relative">
                        ...
                        <Avatar
                          seed={myMatch.avatar_seed}
                          style={myMatch.avatar_style || "avataaars"}
                          size="lg"
                        />
                      </div>
                      <div>
                        <p className="text-[10px] text-yellow-500 font-bold uppercase tracking-widest mb-1">
                          Tu misión es regalar a:
                        </p>
                        <h3 className="text-2xl font-bold text-white">
                          {myMatch.username || "Usuario"}
                        </h3>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => {
                          setActiveTab("others");
                          toast.info(
                            `¡Busca los deseos de ${myMatch.username}!`
                          );
                        }}
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-3xl md:text-4xl font-bold text-text-main mb-1">
                      {selectedGroup.name}
                    </h2>
                ...
                <div className="bg-background/80 backdrop-blur-sm border border-border rounded-xl px-5 py-3">
                  ...
                  <p className="text-2xl font-bold">
                    <span className="text-text-main">{myWishesCount}</span>
              ...
              <div className="mb-6">
                ...
                <div className="h-3 bg-background rounded-full overflow-hidden border border-border relative">
                  ...
                  <div
                    className="h-full bg-gradient-to-r from-purple-500 via-pink-500 to-orange-500 rounded-full transition-all duration-1000 ease-out shadow-lg shadow-purple-500/50 relative overflow-hidden"
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
                <div className="stagger-item hover-lift bg-background/50 backdrop-blur-sm border border-purple-500/20 rounded-xl p-4 hover:border-purple-500/40 transition-all duration-300 shadow-sm hover:shadow-purple-500/10 cursor-default">
                ...
                <div className="stagger-item hover-lift bg-background/50 backdrop-blur-sm border border-pink-500/20 rounded-xl p-4 hover:border-pink-500/40 transition-all duration-300 shadow-sm hover:shadow-pink-500/10 cursor-default">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xl">👥</span>
                    <span className="text-[10px] text-text-muted uppercase tracking-wider">
                      Del Grupo
                ...
                <div className="stagger-item hover-lift bg-background/50 backdrop-blur-sm border border-orange-500/20 rounded-xl p-4 hover:border-orange-500/40 transition-all duration-300 shadow-sm hover:shadow-orange-500/10 cursor-default">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xl">🎯</span>
                    <span className="text-[10px] text-text-muted uppercase tracking-wider">
                      Total
                ...
                <div
                  className={`
                        stagger-item hover-lift relative overflow-hidden group/date rounded-xl p-4 border transition-all duration-300 cursor-default
                        backdrop-blur-sm
                        ${
                ...
                >
                  ...
                  <div
                    className={`absolute -right-4 -top-4 w-16 h-16 rounded-full blur-xl transition-colors ${
                      daysLeft <= 5
                  ...
                  {daysLeft !== null ? (
                    <div>
                      <p className="text-2xl font-bold text-text-main leading-none">
                        {new Date(
                          selectedGroup.event_date + "T00:00:00"
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 mt-10 relative z-10">
        <div className="flex justify-center mb-12">
          <div className="bg-surface p-1 rounded-xl border border-border flex w-full max-w-sm relative shadow-xl">
            <button
              ...
              className={`flex-1 py-3 rounded-lg text-sm md:text-base font-bold transition-all duration-300 relative z-10 flex items-center justify-center gap-2 ${
                activeTab === "mine"
                  ? "text-white"
            ...
            <div
              className={`absolute top-1 bottom-1 w-[calc(50%-4px)] bg-[#6366f1] rounded-lg transition-transform duration-300 ease-out z-0 shadow-[0_0_15px_rgba(99,102,241,0.4)] ${
                activeTab === "mine"
                  ? "translate-x-0"
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-10 tab-content-enter">
          {loadingWishes ? (
            <WishListSkeleton />
          ) : (
            <>
                ...
                <div>
                  ...
                  {filteredWishes.length === 0 ? (
                    <div className="col-span-full text-center py-24 text-text-muted bg-surface/30 rounded-2xl backdrop-blur-sm text-lg">
                      El grupo está silencioso... sé el primero en invitar a
                      alguien.
                    </div>
                  ) : (
                    <WishList
